...
function downloadStory(): void {
  ...
  a.download = `qadr110-${currentData?.countryCode.toLowerCase() || 'story'}-${Date.now()}.png`;
  a.click();
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  const file = new File([currentBlob], `${data.countryCode.toLowerCase()}-qadr110.png`, { type: 'image/png' });
