...
const routes = createSupplyChainServiceRoutes(supplyChainHandler, serverOptions)
  .filter((route) => route.path === '/api/supply-chain/v1/get-chokepoint-status');

if (routes.length !== 1) {
  throw new Error('Exact route wiring failed for /api/supply-chain/v1/get-chokepoint-status');
}
