...
test('uses dedicated parentOrigin for iframe postMessage target', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://qadr.alefba.dev&parentOrigin=https://tauri.localhost'));
  const html = await response.text();
  assert.match(html, /playerVars:\{[^}]*origin:"https:\/\/qadr\.alefba\.dev"/);
...
test('does not accept wildcard parentOrigin query parameter', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://qadr.alefba.dev&parentOrigin=*'));
  const html = await response.text();
  assert.equal(html.includes('parentOrigin="*"'), false);
  assert.match(html, /parentOrigin="https:\/\/qadr\.alefba\.dev"/);
});


test('does not include storage-access bypass behavior in embed shell', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk'));
  const html = await response.text();
  assert.equal(html.includes('requestStorageAccess'), false);
