...
const routes = createIntelligenceServiceRoutes(intelligenceHandler, serverOptions)
  .filter((route) => route.path === '/api/intelligence/v1/search-gdelt-documents');

if (routes.length !== 1) {
  throw new Error('Exact route wiring failed for /api/intelligence/v1/search-gdelt-documents');
}
