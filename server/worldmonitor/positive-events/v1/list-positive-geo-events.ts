...
const LEGACY_CACHE_KEY = 'positive-events:geo:v1';
...
export async function listPositiveGeoEvents(
  ...
  try {
    let raw = await getCachedJson(CACHE_KEY, true) as { events?: PositiveGeoEvent[]; fetchedAt?: number } | null;
    if (!raw) {
      raw = await getCachedJson(LEGACY_CACHE_KEY, true) as { events?: PositiveGeoEvent[]; fetchedAt?: number } | null;
    }
